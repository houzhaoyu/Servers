...
CSession::CSession(boost::asio::io_context& io_context, CServer* server):
	...
	_recv_head_node = std::make_shared<MsgNode>(CHAT_HEAD_TOTAL_LEN);
...
const int CSession::GetUserId() const
{
	return _user_uid;
}
...
void CSession::Send(std::string msg, short msgid) {
	...
	if (send_que_size > CHAT_MAX_SENDQUE) {
		std::cout << "session: " << _session_id << " send que fulled, size is " << CHAT_MAX_SENDQUE << endl;
		return;
	}
		...
		try {
			...
			if (bytes_transfered < total_len) {
				std::cout << "read length not match, read [" << bytes_transfered << "] , total ["
					<< total_len<<"]" << endl;
				Close();
				_server->RemoveSession(_session_id);
				return;
			}
...
{
	auto self = shared_from_this();
	asyncReadFull(CHAT_HEAD_TOTAL_LEN, [self, this](const boost::system::error_code& ec, std::size_t bytes_transfered) {
		try {
			if (ec) {
				std::cout << "handle read failed, error is " << ec.what() << endl;
				Close();
			...
			if (bytes_transfered < CHAT_HEAD_TOTAL_LEN) {
				std::cout << "read length not match, read [" << bytes_transfered << "] , total ["
					<< CHAT_HEAD_TOTAL_LEN << "]" << endl;
				Close();
				_server->RemoveSession(_session_id);
				return;
			}
			...
			short msg_len = 0;
			memcpy(&msg_len, _recv_head_node->_data + HEAD_ID_LEN, CHAT_HEAD_DATA_LEN);
			//网络字节序转化为本地字节序
...
{
	::memset(_data, 0, CHAT_MAX_LENGTH);
