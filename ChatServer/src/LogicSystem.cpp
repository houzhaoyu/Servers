...
namespace {
int GetChatBusinessThreadCount() {
  const auto value =
      ConfigMgr::Inst().GetSelfServer().GetValue("BusinessThreads");
  if (!value.empty()) {
    try {
      return std::clamp(std::stoi(value), 1, 64);
    } catch (const std::exception &) {
      Logger::Error("invalid ChatServer.BusinessThreads: {}", value);
    }
  }
  return 8;
}
...
LogicSystem::LogicSystem() : BaseLogic(GetChatBusinessThreadCount()) {
  RegisterHandlers();
  Start();
  Logger::Info("ChatServer business worker pool started, size = {}",
               GetChatBusinessThreadCount());
}
...
void LogicSystem::RegisterHandlers() {
  _handlers[ID_CHAT_LOGIN] = [this](std::shared_ptr<BaseSession> session,
                                    MsgIdType msg_id,
    ...
    Defer defer([]() { LogContext::Clear(); });
    LoginHandler(s, msg_id, msg_data);
  };

  _handlers[ID_SEARCH_USER_REQ] = [this](std::shared_ptr<BaseSession> session,
                                         MsgIdType msg_id,
                                         const std::string &msg_data) {
    auto s = std::dynamic_pointer_cast<ChatSession>(session);
    if (!s)
  ...
  _handlers[ID_ADD_FRIEND_REQ] = [this](std::shared_ptr<BaseSession> session,
                                        MsgIdType msg_id,
                                        const std::string &msg_data) {
    auto s = std::dynamic_pointer_cast<ChatSession>(session);
    if (!s)
  ...
  _handlers[ID_AUTH_FRIEND_REQ] = [this](std::shared_ptr<BaseSession> session,
                                         MsgIdType msg_id,
                                         const std::string &msg_data) {
    auto s = std::dynamic_pointer_cast<ChatSession>(session);
    if (!s)
  ...
  _handlers[ID_TEXT_CHAT_MSG_REQ] = [this](std::shared_ptr<BaseSession> session,
                                           MsgIdType msg_id,
                                           const std::string &msg_data) {
    auto s = std::dynamic_pointer_cast<ChatSession>(session);
    if (!s)
  ...
  _handlers[ID_HEART_BEAT_REQ] = [this](std::shared_ptr<BaseSession> session,
                                        MsgIdType msg_id,
                                        const std::string &msg_data) {
    auto s = std::dynamic_pointer_cast<ChatSession>(session);
    if (!s)
  ...
  _handlers[ID_LOAD_CHAT_THREAD_REQ] =
      [this](std::shared_ptr<BaseSession> session, MsgIdType msg_id,
             const std::string &msg_data) {
        auto s = std::dynamic_pointer_cast<ChatSession>(session);
        if (!s)
          return;
  ...
  _handlers[ID_LOAD_CHAT_MSG_REQ] = [this](std::shared_ptr<BaseSession> session,
                                           MsgIdType msg_id,
                                           const std::string &msg_data) {
    auto s = std::dynamic_pointer_cast<ChatSession>(session);
    if (!s)
  ...
  _handlers[ID_IMG_CHAT_MSG_REQ] = [this](std::shared_ptr<BaseSession> session,
                                          MsgIdType msg_id,
                                          const std::string &msg_data) {
    auto s = std::dynamic_pointer_cast<ChatSession>(session);
    if (!s)
...
void LogicSystem::LoginHandler(std::shared_ptr<ChatSession> session,
  ...
  RedisMgr::GetInstance()->AsyncGet(
      token_key,
      [this, session, uid, token, uid_str](RedisAsyncResult token_result) {
        PostCallback(
            [this, session, uid, token, uid_str,
             token_result = std::move(token_result)]() mutable {
              Json::Value rtvalue;
              bool async_pending = false;
              Defer defer([this, &rtvalue, &async_pending, session]() {
                if (async_pending) {
                  return;
                }
              ...
              // 用一个 Lua
              // 命令原子读取旧服务器并更新当前登录映射，替代同步分布式锁、GET
              // 和两次 SET。 Redis 网络 I/O 完全在 hiredis async
              // 事件线程执行，当前业务线程在此处立即释放。
              auto server_name =
                  ConfigMgr::Inst().GetSelfServer().GetValue("Name");
              auto uid_ip_key = USER_IP_PREFIX + uid_str;
              auto uid_session_key = USER_SESSION_PREFIX + uid_str;
              auto session_id = session->GetSessionId();
              auto response = rtvalue;
              session->SetUserId(uid);
              async_pending = true;
			  //读取旧服务器名称
			  //并写入当前服务器名和会话ID
              static const std::string bind_login_script =
                  "local old = redis.call('GET', KEYS[1]); "
                  "redis.call('SET', KEYS[1], ARGV[1]); "
                  "redis.call('SET', KEYS[2], ARGV[2]); "
                  "return old or ''";
              RedisMgr::GetInstance()->AsyncCommand(
                  {"EVAL", bind_login_script, "2", uid_ip_key, uid_session_key,
                   server_name, session_id},
                  [this, session, uid, server_name, session_id,
                   response =
                       std::move(response)](RedisAsyncResult result) mutable {
                    PostCallback(
                        [this, session, uid, server_name, session_id,
                         response = std::move(response),
                         result = std::move(result)]() mutable {
                          if (!result.success) {
                            response["error"] = ErrorCodes::RedisReadErr;
                            session->Send(response.toStyledString(),
                                          ID_CHAT_LOGIN_RSP);
                            return;
                          }
                          ...
                          // 客户端可能在异步 Redis
                          // 返回前断开，不能为失效会话留下在线映射。
                          if (!_p_server->CheckValid(session_id)) {
                            session->DealExceptionSession();
                            return;
                          }

                          const auto &old_server = result.value;
                          if (!old_server.empty()) {
                            if (old_server == server_name) {
                              auto old_session =
                                  UserMgr::GetInstance()->GetSession(uid);
                              if (old_session &&
                                  old_session->GetSessionId() != session_id) {
                                old_session->NotifyOffline(uid);
                                old_session->Close();
                                old_session->DealExceptionSession();
                              }
                            } else {
                              KickUserReq kick_req;
                              kick_req.set_uid(uid);
                              ChatGrpcClient::GetInstance()
                                  ->AsyncNotifyKickUser(old_server,
                                                        std::move(kick_req));
                            }
                          }
...
void LogicSystem::SearchInfo(std::shared_ptr<ChatSession> session,
                             const MsgIdType &msg_id,
                             const std::string &msg_data) {
  Json::Reader reader;
  Json::Value root;
...
void LogicSystem::AddFriendApply(std::shared_ptr<ChatSession> session,
                                 const MsgIdType &msg_id,
                                 const std::string &msg_data) {
  Json::Reader reader;
  Json::Value root;
  ...
  Logger::Debug("LogicSystem::AddFriendApply - user AddFriendApply uid is {}",
                uid);
  ...
  // 发送通知
  ChatGrpcClient::GetInstance()->AsyncNotifyAddFriend(to_ip_value,
                                                      std::move(add_req));
}

void LogicSystem::AuthFriendApply(std::shared_ptr<ChatSession> session,
                                  const MsgIdType &msg_id,
                                  const std::string &msg_data) {
  ...
  Logger::Debug("LogicSystem::AuthFriendApply - user fromuid is {} auth friend "
                "touid is {}",
                uid, touid);
  ...
  // 发送通知
  ChatGrpcClient::GetInstance()->AsyncNotifyAuthFriend(to_ip_value,
                                                       std::move(auth_req));
}

void LogicSystem::DealChatTextMsg(std::shared_ptr<ChatSession> session,
                                  const MsgIdType &msg_id,
                                  const std::string &msg_data) {
  Json::Reader reader;
  Json::Value root;
  reader.parse(msg_data, root);
  Logger::Debug(
      "LogicSystem::DealChatTextMsg - receive text chat msg, msg data is {}",
      msg_data);
  ...
  // 数据库入库完成后立即响应发送方；查找接收方路由改为 hiredis async。
  session->Send(rtvalue.toStyledString(), ID_TEXT_CHAT_MSG_RSP);
  auto to_ip_key = USER_IP_PREFIX + std::to_string(touid);
  auto response = rtvalue;
  RedisMgr::GetInstance()->AsyncGet(
      to_ip_key, [this, uid, touid, thread_id, response = std::move(response),
                  chat_datas](RedisAsyncResult route_result) mutable {
        PostCallback(
            [this, uid, touid, thread_id, response = std::move(response),
             chat_datas, route_result = std::move(route_result)]() mutable {
              if (!route_result.success || route_result.value.empty()) {
                return;
              }

              auto self_name =
                  ConfigMgr::Inst().GetSelfServer().GetValue("Name");
              if (route_result.value == self_name) {
                auto target_session = UserMgr::GetInstance()->GetSession(touid);
                if (target_session) {
                  target_session->Send(response.toStyledString(),
              ...
              ChatGrpcClient::GetInstance()->AsyncNotifyTextChatMsg(
                  route_result.value, std::move(text_msg_req));
            },
            std::to_string(uid));
      });
}
...
void LogicSystem::HeartBeatHandler(std::shared_ptr<ChatSession> session,
                                   const MsgIdType &msg_id,
                                   const std::string &msg_data) {
  Json::Reader reader;
  Json::Value root;
  ...
  auto uid = root["fromuid"].asInt();
  Logger::Debug(
      "LogicSystem::HeartBeatHandler - receive heart beat msg, uid is {}", uid);
  Json::Value rtvalue;
  rtvalue["error"] = ErrorCodes::Success;
...
bool LogicSystem::GetBaseInfo(std::string base_key, UserIdType uid,
  ...
  if (b_base) {
    ...
    Logger::Debug("LogicSystem::GetBaseInfo - GetBaseInfo from redis, uid is "
                  "{}, name is {}, email is {}",
                  userinfo->uid, userinfo->name, userinfo->email);
  } else {
    // redis中没有则查询mysql
    // 查询数据库
    std::shared_ptr<UserInfo> user_info = nullptr;
...
bool LogicSystem::GetFriendApplyInfo(
    UserIdType to_uid, std::vector<std::shared_ptr<ApplyInfo>> &list) {
  Logger::Debug(
      "LogicSystem::GetFriendApplyInfo - GetFriendApplyInfo to_uid is {}",
      to_uid);
  // 从mysql获取好友申请列表
...
bool LogicSystem::GetFriendList(
  ...
  Logger::Debug("LogicSystem::GetFriendList - GetFriendList self_id is {}",
                self_id);
  // 从mysql获取好友列表
...
void LogicSystem::GetUserThreadsHandler(std::shared_ptr<ChatSession> session,
                                        ...
                                        const std::string &msg_data) {
  Logger::Debug(
      "LogicSystem::GetUserThreadsHandler - GetUserThreadsHandler msg_id is {}",
      msg_id);
  ...
  int next_last_id = 0;
  bool res =
      GetUserThreads(uid, last_id, page_size, threads, load_more, next_last_id);
  if (!res) {
    rtvalue["error"] = ErrorCodes::UidInvalid;
    return;
  }
...
bool LogicSystem::GetUserThreads(
    UserIdType userId, UserIdType lastId, int pageSize,
    std::vector<std::shared_ptr<ChatThreadInfo>> &threads, bool &loadMore,
    int &nextLastId) {
  Logger::Debug("LogicSystem::GetUserThreads - GetUserThreads userId is {}, "
                "lastId is {}, pageSize is {}",
...
void LogicSystem::CreatePrivateChat(std::shared_ptr<ChatSession> session,
                                    const MsgIdType &msg_id,
                                    const std::string &msg_data) {
  Logger::Debug(
      "LogicSystem::CreatePrivateChat - CreatePrivateChat msg Id is {}",
      msg_id);
  Json::Reader reader;
  Json::Value root;
  reader.parse(msg_data, root);
