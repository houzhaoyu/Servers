...
public:
    ...
    // virtual void Send(const std::string& msg, MsgIdType msgid) = 0;
};
...
public:
    ...
    {
        Logger::Debug("~ProtocolSession destruct, id = {}", _session_id);
    }
    ...
    // 统一发送接口
    // =========================
    void Send(const char *msg, int len, MsgIdType msgid)
    {
        std::lock_guard<std::mutex> lock(_send_lock);

        if (_send_que.size() > Protocol::MAX_SEND_QUE)
        {
            Logger::Debug("ProtocolSession::Send - send queue full, session = {}", _session_id);
            return;
        }
    ...
    void Send(const std::string &msg, MsgIdType msgid)
    {
...
protected:
    ...
    virtual bool ParseHeader(const char *data, MsgIdType &msg_id, int &msg_len) = 0;
...
protected:
    ...
    //将shared_ptr<ProtocolSession<Protocol>>赋值给shared_ptr<BaseSession>
    ...
    void AsyncReadHead(int total_len)
    {
        ...
        asyncReadFull(total_len,
                      ...
                      {
                          if (HandleError(ec, bytes, Protocol::HEAD_LEN))
                              return;
                          ...
                          {
                              Logger::Error("ProtocolSession::AsyncReadHead - parse header failed, session = {}", _session_id);
                              OnError();
                              return;
                          }
                          ...
                          {
                              Logger::Error("ProtocolSession::AsyncReadHead - msg too large, session = {}, msg_len = {}", _session_id, msg_len);
                              OnError();
                              return;
                          }
    ...
    void AsyncReadBody(int total_len)
    {
        ...
        asyncReadFull(total_len,
                      ...
                      {
                          if (HandleError(ec, bytes, total_len))
                              return;
    ...
    void DoWrite()
    {
        ...
        boost::asio::async_write(
            ...
            [this, self](const boost::system::error_code &ec, std::size_t)
            {
                if (ec)
                {
                    Logger::Error("ProtocolSession::DoWrite - write error, session = {}, error = {}", _session_id, ec.message());
                    OnError();
                    return;
                }
                     ...
                     std::size_t expect)
    {
        if (ec)
        {
            Logger::Error("ProtocolSession::HandleError - read error, session = {}, error = {}", _session_id, ec.message());
            OnError();
            return true;
        }
        ...
        if (bytes < expect)
        {
            Logger::Error("ProtocolSession::HandleError - read length mismatch, session = {}, expected = {}, actual = {}", _session_id, expect, bytes);
            OnError();
            return true;
        }
                      ...
                      std::function<void(const boost::system::error_code &, std::size_t)> handler)
    {
        ...
        _socket.async_read_some(
            ...
            [this, self, read_len, total_len, handler](const boost::system::error_code &ec, std::size_t bytes)
            {
                if (ec)
                {
