...
public:
		...
		: url_(url), user_(user), pass_(pass), schema_(schema), poolSize_(poolSize), b_stop_(false), _fail_count(0)
	{
		try
		{
			for (int i = 0; i < poolSize_; ++i)
			{
				...
				Logger::Info("mysql connection init success");
			}
	...
	void checkConnectionPro()
	{
		...
		while (processed < targetCount)
		{
			...
			if (timestamp - con->_last_oper_time >= 5)
			{
				...
				{
					Logger::Error("Error keeping connection alive: {}", e.what());
					healthy = false;
					_fail_count++;
				}
			}
	...
	bool reconnect(long long timestamp)
	{
		try
		{
			...
			Logger::Info("mysql connection reconnect success");
			return true;
		}
		catch (sql::SQLException& e)
		{
			Logger::Error("Reconnect failed, error is {}", e.what());
			return false;
		}
	}
...
public:
	...
	bool TestProcedure(const std::string& email, UserIdType& uid, std::string& name);
	...
	std::shared_ptr<UserInfo> GetUser(UserIdType uid);
	std::shared_ptr<UserInfo> GetUser(std::string name);
	bool GetApplyList(UserIdType touid, std::vector<std::shared_ptr<ApplyInfo>>& applyList, int offset, int limit);
	bool GetFriendList(UserIdType self_id, std::vector<std::shared_ptr<UserInfo>>& user_info);
	bool GetUserThreads(
		UserIdType userId,
		UserIdType lastId,
		int pageSize,
		...
		bool& loadMore,
		int& nextLastId);
	bool CreatePrivateChat(UserIdType user1_id, UserIdType user2_id, int& thread_id);
	...
	bool UpdateHeadInfo(UserIdType uid, const std::string& icon);
